...
( function( global, factory ) {
	...
	if ( typeof module === "object" && typeof module.exports === "object" ) {
		...
		//如果框架不是CommonJS规范，则直接引入实现JQ功能的外部函数
		factory( global );
	}
...
var
	...
	jQuery = function( selector, context ) {
		...
		// Need init if jQuery is called (just allow error to be thrown if not included) - 如果调用了jQuery，就需要init(如果不包括，就允许抛出错误)
...
jQuery.fn = jQuery.prototype = {
	/*
	 *jQuery将对象模拟成了数组，并加上了length, push, sort, splice属性
	 *
	 */
	...
	// The default length of a jQuery object is 0 - jQuery对象的默认长度为0
	length: 0,
...
/*
 *针对遍历，jQuery抽象了一个dir方法，
 *用于根据传递的元素与词素的位置关系，查找指定的元素
 *dir方法传入3个参数：
 *elem为当前节点元素，dir为目标节点元素字符串，until为截止节点元素
 */
...
var dir = function( elem, dir, until ) {
	...
	//while循环中，elem会被循环赋值为当前节点的目标节点元素
	//当elem为真且不为Document节点时，则进入循环
	while ( ( elem = elem[ dir ] ) && elem.nodeType !== 9 ) {
		//这里确保elem为元素节点
		if ( elem.nodeType === 1 ) {
			//如果truncate有赋值且elem是until中的一个，则跳出循环
			//is(expr):用一个表达式来检查当前选择的元素集合，如果其中至少有一个元素符合这个给定的表达式就返回true
			if ( truncate && jQuery( elem ).is( until ) ) {
				break;
			}
			//否则push elem进matched数组
			matched.push( elem );
		}
	}
	//返回元素数组
...
// Give the init function the jQuery prototype for later instantiation - 将jQuery.fn上的所有属性都挂在jQuery实例$(..)s上，是的实例拥有很多jQuery上定义的函数
...
jQuery.each( {
	//parent()接口，获取被选元素的直接父元素
	parent: function( elem ) {
		//获取parent接口
		var parent = elem.parentNode;
		//如果有父节点且父节点不是DocumentFragment，则返回父节点，否则返回null
		return parent && parent.nodeType !== 11 ? parent : null;
	},
	//parents()接口，获取被选元素的所有祖先元素，它一路向上指导文档的根元素
	parents: function( elem ) {
		return dir( elem, "parentNode" );
	},
	//parentsUntil()接口，返回介于两个给定元素之间的所有祖先元素
	...
	//nextAll()接口，返回被选元素的所有跟随的同胞元素
	nextAll: function( elem ) {
		return dir( elem, "nextSibling" );
	},
	//prevAll()接口，返回被选元素前面的同胞元素
	prevAll: function( elem ) {
		return dir( elem, "previousSibling" );
	},
	//nextUntil()接口，返回介于两个给定参数之间的所有跟随的同胞元素（向后遍历）
	nextUntil: function( elem, i, until ) {
		return dir( elem, "nextSibling", until );
	},
	//prevUntil()接口，返回介于两个给定参数之间的所有跟随的同胞元素（向前遍历）
